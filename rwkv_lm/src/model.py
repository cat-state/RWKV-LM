...
wkv_cuda = load(name="wkv", sources=[f"{dir_path}/../cuda/wkv_op.cpp", f"{dir_path}/../cuda/wkv_cuda.cu"],
...
class ImageGPT(GPT):
    def __init__(self, config):
        ...
        old_emb = self.emb
        self.emb = nn.Linear(self.config.vocab_size, self.config.n_embd, bias=False)
        self.emb.weight.data = old_emb.weight.data.T
        self.head = nn.Linear(config.n_embd, config.vocab_size * 10, bias=False)
